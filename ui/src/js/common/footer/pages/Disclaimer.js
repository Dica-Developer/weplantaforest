...
export default class Disclaimer extends Component {
  ...
  componentDidMount() {
    var that = this;
    axios
      .get('http://localhost:8082/articles?articleType=DISCLAIMER&language=' + localStorage.getItem('language'))
      .then(function(response) {
        var result = response.data;
        for (var disclaim in result) {
          axios
            .get('http://localhost:8082/reports/article/' + result[disclaim].id)
            .then(function(response) {
          ...
          <div className="col-md-12">
            <div>
              {this.state.disclaimer.map(function(disclaim, i) {
                return (
                  <div key={i}>
                    <p className="title">{disclaim.title}</p>
                    <p
                      dangerouslySetInnerHTML={{
                        __html: disclaim.intro
                      }}
                    ></p>
                    {disclaim.paragraphs.map(function(paragraph, j) {
                      return (
                        <div key={j} className="link">
                          <p>{paragraph.title}</p>
                          <p
                            dangerouslySetInnerHTML={{
