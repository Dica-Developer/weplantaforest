...
export default class EditUserDetails extends Component {
  ...
  editUser(toEdit, newEntry) {
    ...
    axios
      .post('http://localhost:8081/user/edit?userName=' + encodeURIComponent(this.props.user.userName) + '&toEdit=' + toEdit + '&newEntry=' + encodeURIComponent(newEntry), {}, config)
  ...
  editUsername(newEntry) {
    ...
    axios
      .post('http://localhost:8081/user/edit?userName=' + encodeURIComponent(this.props.user.userName) + '&toEdit=NAME&newEntry=' + encodeURIComponent(newEntry), {}, config)
      .then(function(response) {
        that.props.logout();
      })
      .catch(function(error) {
    ...
    return (
      <div>
        ...
        <EditDropdownItem
          ...
          toEdit="ORGANIZATION_TYPE"
          content={this.props.user.organizationType}
