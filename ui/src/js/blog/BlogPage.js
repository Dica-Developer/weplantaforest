...
export default class BlogPage extends Component {
  ...
  componentDidMount() {
    var that = this;
    axios
      .get('http://localhost:8082/reports/article/' + this.props.params.articleId)
      ...
      <div className="container paddingTopBottom15 blogPage">
        <div className="row">
          ...
          <div className={'col-md-12'}>
            {moment(this.state.article.createdOn).format('DD.MM.YYYY')}
            {' von '}
            <a
              role="button"
              onClick={() => {
                browserHistory.push('/user/' + encodeURIComponent(this.state.article.owner.name));
              }}
            >
