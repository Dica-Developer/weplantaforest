...
export default class Sepa extends Component {
  ...
  render() {
    ...
    if (this.state.paymentDone) {
      header = (
        <div>
          <h1>{counterpart.translate('PAYMENT_SUCCESSFUL')}</h1>
          {counterpart.translate('FOLLOWING_DATA_TRANSFERED')}:
        </div>
      );
      if (JSON.parse(localStorage.getItem('isGift'))) {
        header = (
          <div>
            <h1>{counterpart.translate('PAYMENT_SUCCESSFUL')}</h1>
            {counterpart.translate('YOU_FIND_YOUR_GIFT_CODE')}
            <a role="button" onClick={this.switchToGiftOverview.bind(this)}>
              {' '}
              {counterpart.translate('HERE')}
            </a>
            <br />
            {counterpart.translate('FOLLOWING_DATA_TRANSFERED')}:
          </div>
    ...
    return (
      <div className="payment-data">
        <div className="row">
          ...
          <div className="col-md-8">{header}</div>
          <div className="col-md-2"></div>
        </div>
        <div className="row">
          <div className="col-md-2"></div>
          <div className="col-md-8">
            <div className={'panel panel-warning ' + (this.state.paymentDone ? 'no-display' : '')}>
              ...
              <div className="panel-body">{counterpart.translate('SEPA_HINT_TEXT')}</div>
            </div>
          </div>
          <div className="col-md-2"></div>
        </div>
            ...
            <div className="row">
              <div className="col-md-6">
                <div className="form-group">
                  ...
                  <InputText
                    id="forename"
                    value={this.state.paymentData.forename}
