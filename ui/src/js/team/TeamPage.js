...
export default class TeamPage extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get('http://localhost:8081/trees/team?teamName=' + encodeURIComponent(this.props.params.teamName) + '&page=0&size=15')
  ...
  loadTeamDetails() {
    var that = this;
    axios
      .get('http://localhost:8081/team?teamName=' + encodeURIComponent(this.props.params.teamName))
      .then(function(response) {
        var result = response.data;
        that.setState({
          team: result
  ...
  loadTeamMember() {
    var that = this;
    axios
      .get('http://localhost:8081/team/member?teamName=' + encodeURIComponent(this.props.params.teamName) + '&page=0&size=5')
      .then(function(response) {
        var result = response.data;
        that.setState({
          teamMember: result
  ...
  callPageTeamMember(page) {
    var that = this;
    axios
      .get('http://localhost:8081/team/member?teamName=' + encodeURIComponent(this.props.params.teamName) + '&page=' + page + '&size=5')
      .then(function(response) {
        var result = response.data;
        that.setState({
          teamMember: result
    ...
    if (this.state.edit) {
      teamDetails = (
        <EditTeamDetails
          ...
          loadTeamDetails={this.loadTeamDetails.bind(this)}
          teamNameChangedAction={this.createSuccessMessageAndReloadPage.bind(this)}
        />
      );
    } else {
      teamDetails = (
        <TeamDetails
          team={this.state.team}
          editTeam={this.editTeam.bind(this)}
          deleteAction={this.createDeleteSuccessMessageAndSwitchToProfile.bind(this)}
          loadTeamMember={this.loadTeamMember.bind(this)}
        />
      );
    }
      ...
      treePart = (
        ...
        >
          {this.state.newestPlantRanking.content.map(function(content, i) {
            ...
            return (
              <RankingItem imageUrl={imageUrl} rankNumber={page * 15 + (i + 1)} key={i} showRankNumber="true" showName="true" linkTo={linkTo}>
    ...
    return (
      <div className="container paddingTopBottom15 teamPage">
        <div className="row details">
          <div className="col-md-6 ">{teamDetails}</div>
          <div className="col-md-6">
            <h1>Mitglieder</h1>
            <SmallRankingContainer
              title=""
              withPaging={true}
              rankingType="teamMember"
              page={teamMemberPage}
              callPreviousPage={this.callPreviousPageTeamMember.bind(this)}
              callNextPage={this.callNextPageTeamMember.bind(this)}
              isFirstPage={this.state.teamMember.first}
              isLastPage={this.state.teamMember.last}
            >
