...
public interface TreeRepository extends PagingAndSortingRepository<Tree, Long> {
    ...
    public final static String FIND_TREES_BYUSER_ID_ID_QUERY = "SELECT tree  FROM Tree tree WHERE tree.owner.id = :ownerId";
    
    public final static String FIND_TREES_BY_PPROJECT_ID_ID_QUERY = "SELECT tree  FROM Tree tree WHERE tree.projectArticle.project.id = :projectId";
    ...
    @Query(value = FIND_TREES_BYUSER_ID_ID_QUERY)
    public Page<Tree> findTreesByUserId(@Param("ownerId") Long ownerId, Pageable page);
    
    @Query(value = FIND_TREES_BY_PPROJECT_ID_ID_QUERY)
