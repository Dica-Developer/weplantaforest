...
public interface ProjectReportRepository extends CrudRepository<Project, Long> {
    ...
    @Query(value = FIND_ALL_ACTIVE_PROJECTS_QUERY, countQuery = FIND_ALL_PROJECTS_COUNT_QUERY + " WHERE project.shopActive = true AND project.visible = true")
    List<ProjectReportData> getActiveProjects();

    @Query(value = FIND_ALL_INACTIVE_PROJECTS_QUERY, countQuery = FIND_ALL_PROJECTS_COUNT_QUERY + " WHERE project.shopActive = false AND project.visible = true")
    ...
    @Query(value = "SELECT p FROM Project p where p.visible=true ORDER BY p.shopActive DESC, p.id DESC")
