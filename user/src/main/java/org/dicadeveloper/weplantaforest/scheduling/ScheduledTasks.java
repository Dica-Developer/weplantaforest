...
public class ScheduledTasks {
  ...
  // LogFactory.getLog(ScheduledTasks.class.getName());
  ...
  private void checkCartsForReceipts() {
    ...
    if (carts != null && carts.size() > 0) {
      ...
      for (String userName : userCartMap.keySet()) {
        try {
          // if there are too many receipt receivers, the smtp server throws exceptions
          // like this:
          // com.sun.mail.smtp.SMTPSendFailedException: 432 4.3.2 Concurrent connections
          // limit exceeded.
          // so there is a sleep of 100ms before every mail
          Thread.sleep(500L);
          ...
          _receiptRepository.save(receipt);
          if (sendMails && owner.getMail() != null) {
            // LOG.info("send receipt mail to: " + owner.getUsername());
