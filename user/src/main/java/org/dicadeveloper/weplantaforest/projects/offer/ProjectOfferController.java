...
public class ProjectOfferController {
    ...
    public ResponseEntity<?> offerProject(@Valid @RequestBody ProjectOfferData projectOffer, BindingResult bindingResult) {
        if (!bindingResult.hasErrors()) {
            final String subject = ProjectOfferHelper.createSubject(projectOffer.getName(), projectOffer.getMail());
            String text;

            User user = null;
            if (projectOffer.userId != null) {
                user = _userRepository.findOne(projectOffer.userId);
            }
